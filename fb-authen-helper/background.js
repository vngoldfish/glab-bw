...
// Facebook Auto-Reply & Comment Scraping integration
let extensionWs = null;
function connectExtensionWs() {
    if (extensionWs && (extensionWs.readyState === WebSocket.OPEN || extensionWs.readyState === WebSocket.CONNECTING)) {
        return;
    }
    const wsUrl = "ws://127.0.0.1:8765/ws/extension";
    extensionWs = new WebSocket(wsUrl);

    extensionWs.onopen = () => {
        console.log("Extension WebSocket connected to backend (port 8765)");
    };

    extensionWs.onmessage = (event) => {
        try {
            const msg = JSON.parse(event.data);
            if (msg && msg.action === "reply_comment") {
                // Forward the reply_comment message to facebook content scripts
    ...
    extensionWs.onclose = () => {
        console.log("Extension WebSocket disconnected. Reconnecting in 5s...");
        setTimeout(connectExtensionWs, 5000);
    };

    extensionWs.onerror = (err) => {
...
// Listen for incoming messages from content scripts to forward to the WebSocket
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message && message.action === "fb_post_comments") {
